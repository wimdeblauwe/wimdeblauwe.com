...
<.> We need to return an empty body as we want to replace the `<li>` item on the HTML page with nothing. HTMX interprets an empty response as doing nothing, but a response with nothing is as having to basically delete the target item from the HTML, which is what we want here.
...
This is very similar to what we did for toggling the item completation state. The only difference is that we now use `hx-delete` and a slightly different URL.
...
It does get some getting used to at the start, but once you get it, it is not that hard.
...
See https://github.com/wimdeblauwe/blog-example-code/tree/master/todomvc-htmx[todomvc-htmx] on GitHub for the full sources.
