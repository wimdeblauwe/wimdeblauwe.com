...
If you don't want to do this and keep your domain "pure", then you can create extra JPA specific objects (e.g. `OwnerEntity`, `PetEntity`) in the `repository` package. See https://reflectoring.io/book/[Get Your Hands Dirty on Clean Architecture] for more information on the various ways that you can do mapping between application layers.
